...
def run_experiment(args):
    ...
    from learning.dataset import UCIBinary
    ...
    assert args.dataset in UCIdatasets.keys()
    ...
    dataset = UCIBinary(args.dataset, which_set='train', preproc=preproc)
    valiset = UCIBinary(args.dataset, which_set='valid', preproc=preproc)
    testset = UCIBinary(args.dataset, which_set='test', preproc=preproc)
    ...
    tags.sort()
    expname = "%s-dsgnht-%s-%s-%s-%s"% (args.dataset, "-".join(tags), args.p_model, args.q_model, "-".join([str(s) for s in layer_sizes]))
...
if __name__ == "__main__":
    ...
    parser.add_argument('dataset', default="adult",
        help="UCI binary dataset (Required)")
