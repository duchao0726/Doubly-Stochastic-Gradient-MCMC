...
from models.dgm_vae import VAEDGMLayerStack
from models.vae import VAE, StandardVAETop
...
class Experiment(object):
    ...
    def print_summary(self):
        ...
        for k, v in iteritems(self.trainer.get_hyper_params()):
            ...
            if not isinstance(v, (int, float)):
                if isinstance(v, list):
                    logger.info("  %20s: %s" % (k, ','.join(str(e) for e in v)))
                continue
        ...
        if isinstance(model, VAEDGMLayerStack):
            desc_str = ""
            for layer in model.p_layers:
                if isinstance(layer, VAE):
                    desc_str += str(layer.n_X) + " - "
                    desc_str += "( " + " - ".join(str(e) for e in layer.det_units) + " )"
                if isinstance(layer, StandardVAETop):
                    desc_str += str(layer.n_X)
            logger.info(" %20s: %s" % ("layer sizes", desc_str))
        else:
