...
class DGMLayerStack(Model):
    def __init__(self, **hyper_params):
        super(DGMLayerStack, self).__init__()
    ...
    def get_gradients(self, X, Y, n_samples, layer_discount=1.0):
        ...
        log_like_q = T.sum(T.sum(log_q*w, axis=1))
        ...
        for nl, layer in enumerate(self.q_layers):
            for name, shvar in iteritems(layer.get_model_params()):
                grads_q[shvar] = (layer_discount ** nl) * T.grad(log_like_q, shvar, consider_constant=[w])


        return batch_log_PX, grads_p, grads_q

    def reconstruct(self, X):
        """ Given a set of observed X, sample hidden units and reconstruct X
        """
        p_layers = self.p_layers
        q_layers = self.q_layers
        ...
        # Generate samples of hidden units (feed-forward)
        samples[1], _ = q_layers[0].sample(X)
        # Get samples from generative model given the hidden units
